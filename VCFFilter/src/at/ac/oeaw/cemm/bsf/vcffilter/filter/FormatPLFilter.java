...
/** 
 * Filter for FORMAT field Phred-scaled likelihoods for genotypes (PL) of type Integer array.
 * FormatPLFilter.java 05 APR 2017
 *
 * @author Heiko Müller
 * @version 1.0
 * @since 1.0
 */
public class FormatPLFilter extends FormatArrayFilter{
    /**
    ...
    /**
    * Creates new FormatPLFilter.
    * 
    * @param header VCF header line object
    * @author Heiko Müller
    * @since 1.0
    */
    public FormatPLFilter(VCFFormatHeaderLine header){
        super(header); 
    ...
    public boolean passesFilter(VariantContext vc) { 
        ...
        while(it.hasNext()){
            int[] attributes = it.next().getPL();
            //System.out.println(attribute);
            if(attributes == null){
                return false;
            }    
            for(int i : attributes){
                if(testPredicate(i, operator1, predicate1) || testPredicate(i, operator2, predicate2) || testPredicate(i, operator3, predicate3)){
                    return true;
                }
            }
        }
